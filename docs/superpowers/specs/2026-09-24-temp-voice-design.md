...
- **Leaving means giving the room away**: an owner who leaves their room (even to the creator) loses it, so
  joining the creator always creates a new room.
...
rooms: Map<channelId, {
  ...
  name,                  // the last name the bot applied
  knownNames: string[],  // the last names the bot asked for, to recognise the CHANNEL_UPDATE they cause
  ...
  timer,                 // queued rename: when it fires, the name is computed again from the owner's game
}>
...
1. If a creation is in progress for them → stop.
2. `POST /guilds/{id}/channels`: type 2, `parent_id` = creator's category, `position` = creator's + 1,
   the category's overwrites + the owner overwrite, name from `names.js`.
3. `PATCH /guilds/{id}/members/{user}` `{ channel_id }` to move them.
4. If the move fails (they already left the creator) → delete the room.
...
renames: when 2 happened in the last 10 minutes, it waits until the window allows, then applies the owner's game at that
moment (only the latest one). A 429 answer (another cause) reschedules with its `retry_after`. A queued rename is dropped when
the room is deleted or becomes `manual`.

**Channel update**: for a room, a name that is not in `knownNames` → `manual = true`, queued rename dropped.
Several renames can be in flight, so the last names asked are all recognised, not only the latest.
...
- Discord allows 100 characters in a channel name. The member's name is cut at 32 characters (Discord's own
  limit for names), then the game is cut to fit, with `…`. Lengths are counted in code points.
