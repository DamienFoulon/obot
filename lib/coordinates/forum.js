...
// The original message of a forum post has the id of the post
export function isPostStarter(message) {
  return message.id === message.channel_id;
}

// The bot's panel is recognised by its buttons, even before its id is stored;
// any other message (a music "Now playing" posted there, for instance) goes
export function shouldDeleteMessage(message) {
  if (isPostStarter(message)) return false;
...
// A new post gets its panel with its original message: Discord refuses any message in a post before it (40058)
export async function cleanWorldMessage(message) {
  if (!message.guild_id || !(await isWorldThread(message.channel_id))) return false;
  if (isPostStarter(message)) await ensurePanel(message.channel_id);
