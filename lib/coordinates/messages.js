...
// The card shows the whole note: the list keeps 10 lines under the 4000 characters of a message
const NOTE_PREVIEW = 80;

function shortNote(note) {
  return note.length > NOTE_PREVIEW ? `${note.slice(0, NOTE_PREVIEW - 1)}…` : note;
}
...
export function formatCoordinateLine(coordinate) {
  ...
  if (note) line += ` · *${escapeMarkdown(shortNote(note))}*`;
