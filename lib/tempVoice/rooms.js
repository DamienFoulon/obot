import { OWNER_PERMISSIONS, isNotFound, ownerOverwrite } from './discord.js';
...
export function createRoomRegistry({
  ...
  // The owner of a room: a member overwrite with the owner's rights, that the category doesn't give
  // (a trusted member of the category is copied into every channel of it, rooms and fixed channels alike)
  function findOwner(channel, categoryAllows) {
    const owner = BigInt(OWNER_PERMISSIONS);
    return channel.permission_overwrites?.find((overwrite) => overwrite.type === 1
      && (BigInt(overwrite.allow) & owner) === owner
      && categoryAllows.get(overwrite.id) !== overwrite.allow)?.id;
  }

  // At startup and on each reconnection: no database, a room is a voice channel of the creator's category
  // with an owner. Every room is tracked before the first await: the voice events handled meanwhile find them
  async function restore(guild) {
    ...
    const category = guild.channels.find((channel) => channel.id === creator.parent_id);
    const categoryAllows = new Map((category?.permission_overwrites ?? [])
      .filter((overwrite) => overwrite.type === 1).map((overwrite) => [overwrite.id, overwrite.allow]));

    const found = [];
    for (const channel of guild.channels) {
      if (channel.type !== VOICE || channel.id === creator.id) continue;
      if ((channel.parent_id ?? null) !== (creator.parent_id ?? null)) continue;
      const ownerId = findOwner(channel, categoryAllows);
      if (!ownerId) continue;
      const members = (guild.voice_states ?? [])
        .filter((state) => state.channel_id === channel.id && !bots.has(state.user_id))
        .map((state) => state.user_id);
      const room = rooms.get(channel.id);
      // A reconnection: the room keeps its manual rename and its rename history
      if (room) Object.assign(room, { ownerId, members });
      else track(channel.id, { guildId: guild.id, ownerId, members, name: channel.name });
      found.push(channel.id);
    }
    // Rooms deleted while the bot was disconnected
    for (const [channelId, room] of [...rooms]) {
      if (room.guildId === guild.id && !found.includes(channelId)) forget(channelId);
    }

    for (const channelId of found) {
      const room = rooms.get(channelId);
      if (!room) continue;
      if (room.members.length === 0) await deleteRoom(channelId);
      else if (!room.members.includes(room.ownerId)) await transfer(channelId, room);
      else await scheduleRename(channelId);
    }
  }
