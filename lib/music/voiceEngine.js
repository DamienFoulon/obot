...
import { fetchMissingVoicePermissions } from './voicePermissions.js';
...
class VoiceEngine {
  ...
  // false when the audio player can't pause yet (still buffering)
  pause() {
...
// Discord lets a bot without Speak join: it has to be checked before
async function checkVoicePermissions(guildId, channelId) {
  let missing;
  try {
    missing = await fetchMissingVoicePermissions(guildId, channelId);
  } catch (err) {
    // The join timeout still catches a missing Connect
    console.error(`Cannot check my permissions in the voice channel ${channelId}:`, err.message);
    return;
  }
  if (missing.length) {
    const list = missing.map((name) => `**${name}**`).join(', ');
    throw new VoiceJoinError(`I can't play in <#${channelId}>: I'm missing the ${list} permission${missing.length > 1 ? 's' : ''} there 🔒`);
  }
}
...
export async function connectVoiceEngine({ guildId, channelId }) {
  await checkVoicePermissions(guildId, channelId);
