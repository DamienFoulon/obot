...
export function runYtDlp(args, timeout = 30_000) {
  return new Promise((resolve, reject) => {
    execFile(ytDlpPath(), [...baseArgs(), ...args], { timeout, maxBuffer: 32 * 1024 * 1024 }, (err, stdout, stderr) => {
      ...
      // Without stderr (timeout, output too big), err.message is the command line, cookies path included:
      // it must not reach the members
      reject(new YtDlpError(stderr || (err.killed ? 'yt-dlp took too long to answer' : 'yt-dlp failed')));
    });
  });
}
